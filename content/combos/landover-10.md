If you have a single garage to clear out, a bathroom to gut, or a yard full of brush and downed limbs in Landover, the 10-yard roll-off is almost always the right call. It is the smallest container we run, and that is exactly its strength: a tight 12 ft by 8 ft by 3.5 ft footprint that drops into one standard car spot, handles a one-room project, and never leaves you paying for air. JP Roll-Off Service is a family-owned hauler dispatched out of Beltsville, just up the road, and when you call us at (301) 252-3586 you reach the owners, not a call center.

## What fits in a 10-yard dumpster

The 10-yard holds roughly three pickup-truck loads. Picture three full beds of debris stacked into a low-walled box you can walk right up to and load over the side. It comes with a 1-ton (2,000 lb) weight allowance built into the price, and our typical Landover rate runs $350 to $425 with delivery, pickup, and that ton of tonnage all included. No surprise line items at the end.

Because the walls are low and the footprint is small, this size shines for dense, heavy material in modest volumes rather than big bulky-but-light loads. A bathroom's worth of tile, a garage packed with old paint cans and broken shelving, or a weekend's worth of yard waste fills it sensibly without blowing past the weight cap. If you are gutting a whole house or tearing off a roof, you will want a larger box; for a single room or a single zone of the property, the 10-yard is the honest fit.

## Which Landover jobs suit this size

Landover's housing stock leans heavily toward mid-century subdivisions built roughly between 1955 and 1975 (split-levels, brick ramblers, modest colonials), and that age shows up in the work people actually call about. The 10-yard is the natural container for these one-room projects:

- **Garage cleanout** — the size's best-known job. Decades of stored furniture, lawn equipment, holiday boxes, and the stuff that never made it back inside. One car spot for the dumpster, one garage to empty.
- **Bathroom remodel** — a gut-and-redo on a 50-to-70-year-old bathroom produces a surprising amount of weight in tile, vanity, tub, and old subfloor. The 10-yard's 1-ton allowance is sized right for it.
- **Yard waste and trees** — clearing brush, hauling off a downed limb, or cleaning up an overgrown lot along the Glenarden border or the older subdivisions. Branches and root balls are heavy, so the smaller box keeps you honest on weight.
- **Estate downsize** — clearing out a relative's home a room at a time. Given how many homes here are decades old and how much of the 20785 area sees rental turnover and rehab work, single-room cleanouts are a steady part of what we haul.

If your project grows past one room or you are mixing in heavy demolition debris, give us a call and we will talk you into the right size rather than letting you overfill a 10-yard.

## Local logistics, permits, and drive time

Here is the part Landover folks get tripped up on, and it is worth getting right before the dumpster shows up.

**Most of Landover is unincorporated Prince George's County.** That means if you set the dumpster on your own driveway or private property, you do not need a county permit at all. Placement in the public right-of-way — the street or curb — is a different story: that falls under the Prince George's County Department of Permitting, Inspections and Enforcement (DPIE), which handles temporary-use and site/road permits out of their Largo office. On the larger lots common in the unincorporated subdivisions, driveway placement is usually easy, so most of our customers here never deal with a permit.

**Landover Hills is the exception.** It is a separate incorporated town, platted back in 1940 and incorporated in 1945, with small wartime-era lots and narrow streets. Because it is incorporated, street or right-of-way placement there requires the town's own permit, not the county's — a different process just a few blocks from neighbors who answer to the county instead. And because those WWII-era lots are tight, driveway space can be at a premium, so street placement (and the town permit) comes up more often in Landover Hills than on the bigger unincorporated lots nearby. If you are not sure which set of rules applies to your address, just ask us when you book — we sort it out for you.

For most of the community, including the FedExField corridor near Northwest Stadium, the homes off Route 202 (Landover Road), and the subdivisions toward the Glenarden border, private-property placement keeps things simple. The Route 202 and Ardwick-Ardmore industrial strip tends to generate bigger jobs that outgrow a 10-yard, but for a tradesperson doing a single bathroom or a homeowner clearing a garage in that part of town, this is the box.

As for getting there: Landover is a short hop from our Beltsville yard, an easy run down the Beltway. That closeness is part of why we can be flexible on drop-off and pickup timing and why you are dealing with a local family operation instead of a regional dispatcher who has never driven Landover Road.

## Ready when you are

A 10-yard is the no-overpaying choice for a Landover garage cleanout, bathroom remodel, yard cleanup, or single-room estate downsize. We will confirm whether your address sits in unincorporated county territory or inside Landover Hills, help you decide between driveway and street placement, and get a clean container dropped where you need it. Call the owners directly at (301) 252-3586 — we answer the phone, we know the neighborhoods, and we will make sure you get the right size the first time.
